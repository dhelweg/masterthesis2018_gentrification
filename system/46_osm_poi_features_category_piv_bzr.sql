...
DROP table osm_poi_features_category_piv_bzr;
CREATE table osm_poi_features_category_piv_bzr as 
select 
	planungsraum, 
	timeslice,
	...
	select 
		...
		from (
				...
				from (
					...
					from  (
						select 
							substr(osm.planungsraum,0,length(osm.planungsraum)-2) as planungsraum, 
							timeslice,
							category,
							poi_state, 
							sum(anz) as anz
						from osm_poi_features_category osm
						    group by substr(osm.planungsraum,0,length(osm.planungsraum)-2), 
							timeslice,
							category,
							poi_state
					) w
